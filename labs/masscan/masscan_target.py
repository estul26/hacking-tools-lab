...
<head>
  <meta charset="utf-8">
  <title>Masscan Lab Target</title>
</head>
<body>
  <h1>Masscan Lab Target</h1>
  <p>This local target exposes a few predictable ports for safe scan practice.</p>
</body>
</html>
"""
...
async def handle_ssh(reader, writer):
    writer.write(b"SSH-2.0-OpenSSH_8.9p1 MasscanLab\r\n")
    await writer.drain()
...
async def handle_smtp(reader, writer):
    writer.write(b"220 mail.masscan-lab.local ESMTP LabSMTP\r\n")
    await writer.drain()
    for _ in range(5):
...
async def handle_mysql_banner(reader, writer):
    writer.write(b"\x4a\x00\x00\x00\x0a8.0.36-masscan-lab\x00")
