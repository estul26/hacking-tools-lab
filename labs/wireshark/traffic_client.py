...
def request_http(iteration):
    ...
    connection.request(
        "GET",
        f"/packet-demo?iteration={iteration}",
        headers={"User-Agent": "wireshark-lab-client/1.0"},
    )
...
def talk_smtp():
    with socket.create_connection((TARGET_HOST, 2525), timeout=3) as sock:
        sock.recv(512)
        sock.sendall(b"EHLO client.wireshark-lab.local\r\n")
        sock.recv(1024)
        sock.sendall(b"QUIT\r\n")
        sock.recv(512)
...
def query_dns():
    ...
    question = encode_dns_name("target.wireshark-lab.local") + struct.pack("!HH", 1, 1)
...
def run_iteration(iteration):
    for action in (request_http, talk_smtp, talk_redis, query_dns):
        try:
            if action is request_http:
                action(iteration)
            else:
                action()
