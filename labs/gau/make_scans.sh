...
BASE_URL="${BASE_URL:-http://172.55.250.20:8080}"
...
BLOCKED_EXTENSIONS="/work/scope/blocked-extensions.txt"
...
gau --help > "$OUTPUT_DIR/help.txt" 2>&1 || true
gau --version > "$OUTPUT_DIR/version.txt" 2>&1 || true

cat > "$OUTPUT_DIR/local-archive-note.txt" <<NOTE
This local lab uses a deterministic gau-shaped fixture for $DOMAIN.
gau queries public URL sources such as Wayback, Common Crawl, OTX, and URLScan,
and Docker-only training domains do not exist in those sources. Use the real
binary for help/version and authorized public-domain workflows, then practice
local URL parsing and validation here.
NOTE
...
with open(sys.argv[1], encoding="utf-8") as source, open(sys.argv[2], "w", encoding="utf-8") as out:
    out.write("url\tscheme\thost\tpath\textension\tquery\n")
    for line in source:
        url = line.strip()
        if not url:
            continue
        ...
        out.write(f"{url}\t{parsed.scheme}\t{parsed.hostname or ''}\t{path}\t{extension}\t{parsed.query}\n")
PY

awk -F '\t' 'NR > 1 {print $3}' "$OUTPUT_DIR/parsed.tsv" | sort -u > "$OUTPUT_DIR/hosts.txt"
awk -F '\t' 'NR > 1 && $6 != "" {print $1}' "$OUTPUT_DIR/parsed.tsv" | sort -u > "$OUTPUT_DIR/parameterized-urls.txt"
awk -F '\t' 'NR > 1 && $5 != "" {print $5}' "$OUTPUT_DIR/parsed.tsv" | sort -u > "$OUTPUT_DIR/extensions.txt"
sort -u "$ALLOW_HOSTS" > "$OUTPUT_DIR/allow-hosts.sorted.txt"
sort -u "$BLOCKED_EXTENSIONS" > "$OUTPUT_DIR/blocked-extensions.sorted.txt"
comm -12 "$OUTPUT_DIR/allow-hosts.sorted.txt" "$OUTPUT_DIR/hosts.txt" > "$OUTPUT_DIR/in-scope-hosts.txt"

python3 - "$OUTPUT_DIR/urls.txt" "$OUTPUT_DIR/in-scope-hosts.txt" "$OUTPUT_DIR/blocked-extensions.sorted.txt" "$OUTPUT_DIR/in-scope-urls.txt" <<'PY'
import sys
from urllib.parse import urlparse

with open(sys.argv[2], encoding="utf-8") as allowed_file:
    allowed = {line.strip() for line in allowed_file if line.strip()}
with open(sys.argv[3], encoding="utf-8") as blocked_file:
    blocked = {line.strip().lower() for line in blocked_file if line.strip()}

with open(sys.argv[1], encoding="utf-8") as source, open(sys.argv[4], "w", encoding="utf-8") as out:
    for line in source:
        url = line.strip()
        parsed = urlparse(url)
        path = parsed.path or "/"
        last = path.rsplit("/", 1)[-1]
        extension = last.rsplit(".", 1)[-1].lower() if "." in last else ""
        if parsed.hostname in allowed and extension not in blocked:
            out.write(url + "\n")
PY
...
PY
    ...
    code="$(curl -sS -H "Host: $host" -o /tmp/gau-lab-body -w "%{http_code}" "$target_url")"
    printf '%s\t%s\n' "$url" "$code"
